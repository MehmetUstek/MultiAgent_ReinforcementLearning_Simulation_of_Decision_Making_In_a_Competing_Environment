...
from training.uniform_experience_replay import Memory as UER
from training.prioritized_experience_replay import Memory as PER
...
class Agent(object):
    ...
    def __init__(self, state_size, action_size, index, brain_name, arguments):
        self.state_size = state_size
        self.action_size = action_size
        self.index = index
        self.learning_rate = arguments['learning_rate']
        self.gamma = 0.95
        # TODO: Get the same brain. Not different for every agent.
    ...
    def find_targets_per(self, batch):
        ...
        for i in range(batch_len):
            ...
            old_value = t[a]
            if all(done):
                # TODO: Cumulative sum of all agents, maybe not the most likely approach?
                # t[a] = sum(r)
                t[a] = r[self.index]
            else:
                if self.target_type == 'DDQN':
                    # t[a] = sum(r) + self.gamma * pTarget_[i][np.argmax(p_[i])]
                    t[a] = r[self.index] + self.gamma * pTarget_[i][np.argmax(p_[i])]
                elif self.target_type == 'DQN':
                    # t[a] = sum(r) + self.gamma * np.amax(pTarget_[i])
                    t[a] = r[self.index] + self.gamma * np.amax(pTarget_[i])
                else:
            ...
            # errors[i] = np.abs(t[a]) - np.abs(old_value)
            # print(errors)
        return [x, y, errors]
